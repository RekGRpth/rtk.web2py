create or replace function pdrhpfcticatictic_update() returns table (command text, count bigint) as $body$
with s as (
    with s as (
        select      provider,
                    ...
                    providers,
                    flats,
                    round(flats::numeric / providers::numeric, 2) as capacity,
        ...
        group by    1, 4, 5, 6
                ...
                round(capacity - telephone::numeric, 2) as telephone_free,
                round(capacity - inet_dynamic_ip::numeric, 2) as inet_dynamic_ip_free,
                round(capacity - cable_tv::numeric, 2) as cable_tv_free,
                round(telephone::numeric / capacity * 100 , 2)as telephone_penetration,
                round(inet_dynamic_ip::numeric / capacity * 100, 2) as inet_dynamic_ip_penetration,
                round(cable_tv::numeric / capacity * 100, 2) as cable_tv_penetration,
                now() as date
    from        s
    --limit       10
), u as (
    update only pdrhpfcticatictic as t set
        provider = s.provider,
    ...
    insert into pdrhpfcticatictic select s.* from s left join u using (house) where u.house is null returning house, 'insert'::text as command
